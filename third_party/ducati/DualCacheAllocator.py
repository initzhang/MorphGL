...
def get_slope(args, graph, counts, seeds_list, all_data):
    ...
    adj_counts, nfeat_counts = counts
    adj_factor = (args.total_budget/2 * 1024 ** 3) / ((graph.num_edges() + graph.num_nodes() + 1) * 8.)
    nfeat_factor = (args.total_budget/2 * 1024 ** 3) / (graph.num_nodes() * (args.fake_dim + 1) * 4.)
    ...
    for cached_nnz_adj_ratio in adj_ratio_steps:
        ...
        if cache_size == 0:
            ...
            try:
                ...
                mlog(f'early stop at adj due to {e}')
                break
        # warmup
    ...
    del cached_indptr, cached_indices
    ...
    def retrieve_data(cpu_data, gpu_data, idx, out_buf):
        ...
        if gpu_map is None:
            ...
            cur_res[gpu_mask] = gpu_data[gpu_local_nids].to(cur_res.dtype)
            cur_res[~gpu_mask] = cpu_data[cpu_nids].to(cur_res.dtype)
        return cur_res
